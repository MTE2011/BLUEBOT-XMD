...
bluebot({
  ...
  try {
    exec("npx pm2 status BLUEBOT-v2", async (err, stdout, stderr) => {
      if (err) {
      ...
      const lines = clean.split('\n').filter(line => line.includes('BLUEBOT-v2'))
      if (lines.length === 0) {
        await m.send("No BLUEBOT processes found running.")
        return
      }
...
cmd: "repo|sc|script",
  ...
  try {
    const msg =
    `╔═════《 My Repository 》═════╗
╠ Link: https://github.com/M3264/BLUEBOT-Ai
