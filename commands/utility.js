...
bluebot({
    ...
    try {
        ...
        var res = await m.axios(
            `https://api.BLUEBOT.live/api/translate?text=${encodeURIComponent(t)}&to=${code}`
        )
...
cmd: "ngl",
  ...
  try {
    ...
    var res = await m.axios(`https://BLUEBOT-api.vercel.app/ngl?username=${encodeURIComponent(user)}&message=${encodeURIComponent(msg)}`)
    if (res?.success) {
