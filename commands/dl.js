...
blue.bot.push(
    ...
    },
    {
        ...
        category: "downloader",
        async execute(sock, msg, { from, text }) {
            if (!text) return sock.sendMessage(from, { text: "Please provide a Facebook video link." }, { quoted: msg });
            try {
                const { data } = await axios.get(`https://api.kord.live/api/fb?url=${encodeURIComponent(text)}`);
                if (!data.url) return sock.sendMessage(from, { text: "Video not found." }, { quoted: msg });
                await sock.sendMessage(from, { video: { url: data.url }, caption: config.FOOTER || "BLUEBOT-XMD" }, { quoted: msg });
            } catch (e) {
                sock.sendMessage(from, { text: "Error downloading Facebook video." }, { quoted: msg });
            }
        }
    },
    {
        ...
        category: "downloader",
        async execute(sock, msg, { from, text }) {
            if (!text) return sock.sendMessage(from, { text: "Please provide a TikTok link." }, { quoted: msg });
            try {
                const { data } = await axios.get(`https://api.kord.live/api/tiktok?url=${encodeURIComponent(text)}`);
                if (!data.url) return sock.sendMessage(from, { text: "Video not found." }, { quoted: msg });
                await sock.sendMessage(from, { video: { url: data.url }, caption: config.FOOTER || "BLUEBOT-XMD" }, { quoted: msg });
            } catch (e) {
                sock.sendMessage(from, { text: "Error downloading TikTok video." }, { quoted: msg });
            }
        }
    },
    {
        ...
        category: "downloader",
        async execute(sock, msg, { from, text }) {
            if (!text) return sock.sendMessage(from, { text: "Please provide an Instagram link." }, { quoted: msg });
            try {
                const { data } = await axios.get(`https://api.kord.live/api/insta?url=${encodeURIComponent(text)}`);
                if (!data.url) return sock.sendMessage(from, { text: "Media not found." }, { quoted: msg });
                await sock.sendMessage(from, { video: { url: data.url }, caption: config.FOOTER || "BLUEBOT-XMD" }, { quoted: msg });
            } catch (e) {
                sock.sendMessage(from, { text: "Error downloading Instagram media." }, { quoted: msg });
            }
        }
    }
);
