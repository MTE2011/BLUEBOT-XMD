...
cmd: "apk",
  ...
  try {
    ...
    var data = await m.axios(`https://api.BLUEBOT.live/api/apk?q=${text}`)
...
cmd: "apksearch",
  ...
  try {
    ...
    var data = await m.axios(`https://api.BLUEBOT.live/api/apkdl?id=${q}`)
    if (data.error) return await m.send("_Apk not found.._")
    ...
    var info = await m.axios(`https://api.BLUEBOT.live/api/apksearch?query=${text}`);
...
bluebot({
  ...
  let data;
  try {
    data = await m.axios(`https://api.BLUEBOT.live/api/subtitle?q=${text}`);
  } catch (e) {
...
cmd: "subtitlesearch|subtitles",
  ...
  try {
    ...
    let data = await m.axios(`https://api.BLUEBOT.live/api/subtiledl?q=${encodeURIComponent(pageUrl)}`);
    ...
    const info = await m.axios(`https://api.BLUEBOT.live/api/subtitlepage?q=${text}`);
...
bluebot({
        ...
        try {
                ...
                const ttregex = /https:\/\/(?:www\.|vm\.|m\.|vt\.)?tiktok\.com\/(?:(@[\w.-]+\/(?:video|photo)\/\d+)|v\/\d+\.html|[\w-]+\/?)(?:\?.*)?$/
                var link = links.find(url => ttregex.test(url));
                const dta = await fetch(`https://api.BLUEBOT.live/api/tik-img?url=${encodeURIComponent(link)}`);
...
bluebot({
        ...
        try {
                ...
                const api = `https://api.BLUEBOT.live/api/pinterest?url=${encodeURIComponent(link)}`
