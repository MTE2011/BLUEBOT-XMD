...
cmd: "black",
  ...
  try {
    ...
    const blackImg = await getBuffer("https://cdn.BLUEBOT.live/serve/n2BwUtItyeae.jpg")
...
cmd: "roundstk|round",
  ...
  try {
    ...
    categories: ["🤩", "🎉"],
    id: "https://github.com/M3264/BLUEBOT-Ai",
    quality: 75,
    })
...
bluebot({
  ...
  if (m.quoted.sticker) {
    ...
    if (text) {
      ...
      data = config().AUDIO_DATA.split(";") || ["BLUEBOT", "v2", ""]
    }
    
    const title = data[0] || "BLUEBOT"
    const artist = data[1] || "v2"
    const coverUrl = data[2] || "https://cdn.BLUEBOT.live/serve/tuNyPANPYD2v.png"
    
    try {
      const audioBase64 = audioResult.toString('base64')
      
      const response = await fetch('https://api.BLUEBOT.live/add-mp3-meta', {
        method: 'POST',
        headers: {
